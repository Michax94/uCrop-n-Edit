...
constructor(
    ...
    fun setCropGridRowCount(
        @IntRange(from = 0) cropGridRowCount: Int,
    ) {
    ...
    fun setCropGridColumnCount(
        @IntRange(from = 0) cropGridColumnCount: Int,
    ) {
    ...
    fun setupCropBounds() {
        ...
        if (height > mThisHeight) {
            ...
            cropViewRect[
                (paddingLeft + halfDiff).toFloat(), paddingTop.toFloat(),
                (
                        paddingLeft + width + halfDiff
                        ).toFloat(),
            ] =
    ...
    private fun updateGridPoints() {
        ...
        mCircularPath.addCircle(
            cropViewRect.centerX(),
            cropViewRect.centerY(),
            min(cropViewRect.width(), cropViewRect.height()) / 2f,
            Path.Direction.CW,
        )
    }
     ...
     * 3<-------2
     */
    private fun updateCropViewRect(
        touchX: Float,
        touchY: Float,
    ) {
        mTempRect.set(cropViewRect)

        if (mFreestyleCropMode == FREESTYLE_CROP_MODE_ENABLE_WITH_ASPECT_RATIO) {

            when (mCurrentTouchCornerIndex) {
                4 -> {
                    mTempRect.offset(touchX - mPreviousTouchX, touchY - mPreviousTouchY)
                    if (mTempRect.left >= paddingLeft && mTempRect.top >= paddingTop &&
                        mTempRect.right <= width - paddingRight && mTempRect.bottom <= height - paddingBottom) {
                ...
                in 0..3 -> {
                    calculateAspectRatioConstrainedRect(
                        mTempRect, touchX, touchY, mCurrentTouchCornerIndex
                    )

                    val fitsInBounds = mTempRect.left >= paddingLeft &&
                            mTempRect.top >= paddingTop &&
                            mTempRect.right <= width - paddingRight &&
                    ...
                    if (fitsInBounds && changeHeight && changeWidth) {
                        cropViewRect.set(mTempRect)
                        updateGridPoints()
                        postInvalidate()
                    }
     ...
     */
    private fun getCurrentTouchIndex(
        touchX: Float,
        touchY: Float,
    ): Int {
        ...
        return if ((mFreestyleCropMode == FREESTYLE_CROP_MODE_ENABLE ||
                    mFreestyleCropMode == FREESTYLE_CROP_MODE_ENABLE_WITH_ASPECT_RATIO ||
                    mFreestyleCropMode == FREESTYLE_CROP_MODE_ENABLE_WITH_PASS_THROUGH) &&
            closestPointIndex < 0 &&
            cropViewRect.contains(touchX, touchY)) {
            4
        } else {
            closestPointIndex
        }
    ...
    fun processStyledAttributes(a: TypedArray) {
        ...
        mDimmedColor =
            a.getColor(
                ...
                ContextCompat.getColor(this.context, R.color.ucrop_color_default_dimmed),
            )
    ...
    private fun initCropFrameStyle(a: TypedArray) {
        ...
        val cropFrameColor =
            a.getColor(
                ...
                ContextCompat.getColor(this.context, R.color.ucrop_color_default_crop_frame),
            )
    ...
    private fun initCropGridStyle(a: TypedArray) {
        ...
        val cropGridColor =
            a.getColor(
                ...
                ContextCompat.getColor(this.context, R.color.ucrop_color_default_crop_grid),
            )
    ...
    private fun calculateAspectRatioConstrainedRect(
        tempRect: RectF,
        touchX: Float,
        touchY: Float,
        cornerIndex: Int
    ) {
        val aspectRatio = mTargetAspectRatio

        when (cornerIndex) {
            0 -> tempRect[touchX, touchY, cropViewRect.right] = cropViewRect.bottom
            1 -> tempRect[cropViewRect.left, touchY, touchX] = cropViewRect.bottom
            2 -> tempRect[cropViewRect.left, cropViewRect.top, touchX] = touchY
            3 -> tempRect[touchX, cropViewRect.top, cropViewRect.right] = touchY
        }

        if (aspectRatio <= 0) return


        val currentWidth = tempRect.width()
        val currentHeight = tempRect.height()
        val currentRatio = currentWidth / currentHeight

        if (abs(currentRatio - aspectRatio) < 0.01f) return

        val newHeight = currentWidth / aspectRatio
        val newWidth = currentHeight * aspectRatio

        val heightDiff = abs(newHeight - currentHeight)
        val widthDiff = abs(newWidth - currentWidth)

        if (heightDiff < widthDiff) {
            when (cornerIndex) {
                0, 1 -> {
                    tempRect.top = tempRect.bottom - newHeight
                }
                2, 3 -> {
                    tempRect.bottom = tempRect.top + newHeight
                }
            }
        } else {
            when (cornerIndex) {
                0, 3 -> {
                    tempRect.left = tempRect.right - newWidth
                }
                1, 2 -> {
    ...
    @IntDef(
        ...
        FREESTYLE_CROP_MODE_ENABLE_WITH_ASPECT_RATIO,
    )
    ...
    companion object {
        ...
        const val FREESTYLE_CROP_MODE_ENABLE_WITH_ASPECT_RATIO = 3
